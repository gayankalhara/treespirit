...
FILE * wavfile_open( const char *filename )
{
	...
	int bits_per_sample = 8; // changed from 16
...
void wavfile_write( FILE *file, uint8_t data[], int length ) // changed short to uint8_t
{
	fwrite(data,sizeof(uint8_t),length,file); // changed short to uint8_t
}
